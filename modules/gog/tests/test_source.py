...
def test_library(src, env, capsys, monkeypatch):
    ...
    assert mini == {"event": "game", "id": "1434554947", "title": "Mini Metro", "owned": True, "installed": True,
                    ...
                    "release_year": 2015, "image": "https://images-2.gog-statics.com/abc.jpg"}
    assert drift["installed"] is False and drift["dir"] is None and drift["release_year"] is None
    assert json.loads((env["data"] / "library.json").read_text()) == LIBRARY_PAGE["products"]
...
def test_library_offline_uses_cache(src, env, capsys, monkeypatch):
    seed_cache(src, env)
    ...
    assert "cached library" in err


def test_library_offline_without_cache_fails(src, env, capsys, monkeypatch):
    fake_fetch(monkeypatch, src, {"https://embed.gog.com/account/getFilteredProducts": src.SourceError("offline")})
    code, events, _ = run(src, capsys, "library")
    assert code == 1 and events == []
...
def test_info(src, env, capsys):
    ...
    assert code == 0
    assert events == [{"event": "info", "data": {"buildId": "B2", "folder_name": "Mini Metro",
                                                  "size": {"en-US": {"disk_size": 1000}}}}, {"event": "done"}]
