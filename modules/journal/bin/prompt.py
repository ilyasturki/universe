...
LOCATING_WEB = (
    ...
    "name places, characters, bosses and chapters correctly, and tell a main quest from a side quest. For a game without linear progression (sandbox, "
    "rhythm game, abstract) or without a findable guide, skip this step. Never draw from it an event the player has not reached yet and do not spoil what comes next."
)
LOCATING_OFFLINE = (
    "Locating: you have no web access for this entry. Place the session from the images and the memory alone, and name a place, character, boss or chapter "
    "only where an image shows the name or the memory already holds it. Anywhere else, stay generic: a name you infer from what the game probably contains is an invention."
)
...
def system_prompt(forced_lang=None, web=True, override=""):
    """The built-in prompt, or `override` in its place; the language and web rules are appended either way."""
    base = override.strip() if override and override.strip() else SYSTEM_PROMPT
    if not web:
        base = base.replace(LOCATING_WEB, LOCATING_OFFLINE) if LOCATING_WEB in base else f"{base}\n{LOCATING_OFFLINE}"
    if not forced_lang:
        return base
...
def build_user_prompt(title, start, end, duration_s, session_number, total_sec, images, prev, memory, web=True):
    mem_ctx = memory_context(memory)
    lookup = " briefly search the web for the game to establish the premise and the spelling of the main names" if web else " work from the images alone"
    no_memory_note = (
        f"This is your first entry for this game:{lookup}."
        if session_number == 1
        else f"No game memory has been recorded yet, but this is NOT your first session on this game (see the history above and the previous entry): "
        f"do not present this session as a start or a restart, and{lookup}."
    )
    ...
    return "\n".join(
        [
            ...
            *image_lines(images),
            ...
            "",
            "First place this session in the game's progression"
            + (" (use the web and a guide if the game has one)" if web else " from the images and the memory")
            + ", then sort the images, give the session a title, write the journal entry and update the memory.",
        ]
    )
