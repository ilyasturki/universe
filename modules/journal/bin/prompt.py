...
OUTPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "body": {"type": "string", "description": "The journal body, in the game's language: shape A (prose only) or shape B (prose then a bulleted list). Without the pick-up line."},
...
def build_user_prompt(title, start, end, duration_s, session_number, total_sec, images, prev, memory):
    ...
    no_memory_note = (
        ...
        "No game memory has been recorded yet, but this is NOT your first session on this game (see the history above and the previous entry): do not present this session as a start or a restart, and briefly search the web for the game's premise and the spelling of the names."
    )
    ...
    return "\n".join([
        ...
        *image_lines(images),
        ...
        if profile else
        "No profile is recorded for this game yet: decide whether it is \"narrative\" or \"arcade\", write it in the memory, and write this entry in the matching shape.",
        "",
