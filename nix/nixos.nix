...
in {
  options.programs.universe = {
    ...
    inputplumber.enable = lib.mkOption { type = lib.types.bool; default = true; description = "The InputPlumber daemon, which the emulators' `inputplumber` option uses to hand the pads to the emulator as one composite device for the session."; };
